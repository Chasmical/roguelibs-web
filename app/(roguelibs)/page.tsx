...
import getItemData from "@app/assets/sor1-v98/item-data.json";
...
export default async function LandingPage() {
  ...
  const items = sample(data.entries, 10);
      ...
      <MdxContainer>
        <Tabs query="os">
          <TabItem value="win" label="Windows" icon="copy">
            ...
            <p>
              {items.map(({ invItemName: name, spriteName }) => (
                <img
                  key={name}
                  data-tooltip-id={"item-" + name}
                  src={`/assets/sor1-v98/sprites/Items/${spriteName}.png`}
                  alt={name}
                />
              ))}
            </p>
            {items.map(({ invItemName: name }) => (
              <Tooltip key={name} id={"item-" + name} content={name!} />
            ))}
